  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {analysis && title && settings && <>
        <section className="disc-summary"><div><span>原盘</span><strong>{analysis.discName}</strong><small>{analysis.playlistCount} 个播放列表 · 找到 {analysis.titles.length} 个有效标题</small></div><div className="stat"><span>{title.suspiciousLoop ? '异常播放列表' : '当前标题'}</span><b>{formatPlaylistDuration(title)}</b><small>{title.suspiciousLoop ? '不会参与推荐或转码' : `${formatBytes(title.sizeBytes)} · ${title.clips.length} 个片段`}</small></div><div className="stat"><span>画面</span><b>{title.video.width}×{title.video.height}</b><small>{formatHdr(title)} · {title.video.fps.toFixed(3)} fps</small></div></section>
        ...
        <section className="workspace">
          <aside><h3>标题 / 播放列表</h3>{analysis.titles.slice(0, 30).map((item) => <button className={`${item.id === title.id ? 'title-item active' : 'title-item'}${item.suspiciousLoop ? ' suspicious' : ''}`} key={item.id} disabled={busy} onClick={() => void loadTitle(item)}><b>{item.playlistName}{item.id === analysis.recommendedTitleId && <em>推荐</em>}{item.suspiciousLoop && <em>循环/异常</em>}</b><span>{formatPlaylistDuration(item)}{item.suspiciousLoop ? '' : ` · ${formatBytes(item.sizeBytes)}`}</span>{item.duplicateOf && <small>与 {item.duplicateOf} 重复</small>}</button>)}</aside>
...
function formatPlaylistDuration(title:BlurayTitle){return title.suspiciousLoop?'循环播放 / 异常时长':formatTime(title.durationSeconds)}
